...
class EXTRA(DGMBase):
    ...
    def _stateInit(self, X0):
        ...
        G0 = D(self.F(X0), X0)
        with torch.no_grad():
            Y = self._consensusUpdate(X0)
            X1 = Y - self.eta*G0
        self._record(X1, 1)
        del self.logs['nmix'][-1]
        self._k += 1
...
class DIGing(DGMBase):
    ...
    def _step(self, X0, G0, Y0):
        with torch.no_grad():
            WX, WY = self._consensusUpdate(X0, Y0)
            X1 = WX - self.eta*Y0
...
class DSGD(DGMBase):
    ...
    def _step(self, X0):
        ...
        G = D(self.F(X0), X0)
        with torch.no_grad():
            X1 = self._consensusUpdate(X0-self.eta*G)
...
class Mudag(DGMBase):
    ...
    def _step(self, X0, Y0, G0, Y1):
        ...
        with torch.no_grad():
            X1 = Y1 + X0 - Y0 - self.eta*(G1-G0)
            X1 = self._consensusUpdate(X1)
